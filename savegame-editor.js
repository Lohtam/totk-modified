...
window.addEventListener('load', function(){
	...
	dragMessage.innerHTML='<button class="close" onclick="document.getElementById(\'file-load\').click()"><i class=\"icon disk\"></i> Browse progress.sav or caption.sav,</button> drop it here, or drop the entire save folder here.';
	...
	if(!SavegameEditor.noDemo){
		var demoMessagetwo=document.createElement('button');
		var demoMessage=document.createElement('button');
		var demoMessagethree=document.createElement('button');
		var demoMessagefour=document.createElement('button');
		var demoMessagefive=document.createElement('button');
		demoMessage.id='demo';
		demoMessagetwo.id='demo';
		demoMessagethree.id='demo';
		demoMessagefour.id='demo';
		demoMessagefive.id='demo';
		demoMessage.innerHTML='Click Here for a save at the start of the Great Sky Island.';
		demoMessagetwo.innerHTML='Click Here for a save at the start of prologue.';
		demoMessagethree.innerHTML='Click Here for a save at the end of the Great Sky Island.';
		demoMessagefour.innerHTML='Click Here for a save at the Ultrabroken spot.';
		demoMessagefive.innerHTML='Click Here for a maxed out 100% save file.';
		demoMessage.addEventListener('click', function(){
			var filename='progress.sav'; // GSI Start
		...
		demoMessagetwo.addEventListener('click', function(){
			var filename='1.sav'; // Prologue
			if(typeof window.fetch==='function'){
				fetch(filename)
					.then(res => res.arrayBuffer()) // Gets the response and returns it as a blob
					.then(ab => {
		...
		demoMessagethree.addEventListener('click', function(){
			var filename='2.sav'; // End of GSI 
			if(typeof window.fetch==='function'){
				fetch(filename)
					.then(res => res.arrayBuffer()) // Gets the response and returns it as a blob
					.then(ab => {
		...
		demoMessagefour.addEventListener('click', function(){
			var filename='3.sav'; // UB File
			if(typeof window.fetch==='function'){
				fetch(filename)
					.then(res => res.arrayBuffer()) // Gets the response and returns it as a blob
					.then(ab => {
		...
		demoMessagefive.addEventListener('click', function(){
			var filename='4.sav'; // 100% File
			if(typeof window.fetch==='function'){
				fetch(filename)
					.then(res => res.arrayBuffer()) // Gets the response and returns it as a blob
					.then(ab => {
		...
		dragZone.appendChild(demoMessagetwo);
		dragZone.appendChild(demoMessage);
		dragZone.appendChild(demoMessagethree);
		dragZone.appendChild(demoMessagefour);
		dragZone.appendChild(demoMessagefive);
